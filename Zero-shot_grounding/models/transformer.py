...
#     def __init__(self, decoder_layer_high, decoder_layer_low, num_layers, multi=True, norm=None, return_intermediate=False):
#         super().__init__()
#         self.high_layers = _get_clones(decoder_layer_high, num_layers)
#         self.multi = multi
#         if multi:
#             self.low_layers = _get_clones(decoder_layer_low, num_layers)
#         self.num_layers = num_layers
#         self.norm = norm
#         self.return_intermediate = return_intermediate

#     def forward(self, tgt, high_memory, low_memory=None, low_level_idx=None, high_level_idx=None,
#                 tgt_mask: Optional[Tensor] = None,
#                 memory_mask: Optional[Tensor] = None,
#                 tgt_key_padding_mask: Optional[Tensor] = None,
#                 memory_key_padding_mask: Optional[Tensor] = None,
...
#         '''
#         if not self.multi:
#             high_level_idx = range(tgt.shape[0])
#             low_level_idx = []
#         output = tgt
#         N = tgt.shape[1]
#         T,B,d = high_memory.shape
#         intermediate = []
#         atten_layers = []
#         if self.multi:
#             ws =  [torch.zeros(B, tgt.shape[0], low_memory.shape[0]).to(tgt.device)]*4
#         else:
#             ws = [torch.zeros(B, tgt.shape[0], T).to(tgt.device)]*4
#         for n,layer in enumerate(self.high_layers):
#             outputs = torch.zeros_like(tgt).to(tgt.device)
#             residual=True
#             ## Visual-grounded feature learning - output [B, N_disease, dim], ws [B, N_disease, N_visual]
#             output,ws = layer(output, high_memory, low_memory, tgt_mask=tgt_mask,
#                            memory_mask=memory_mask,
...
#             # if self.multi:
#             #     output_low, ws_low = self.low_layers[n](output[low_level_idx], low_memory, tgt_mask=tgt_mask,
#             #                memory_mask=memory_mask,
#             #                tgt_key_padding_mask=tgt_key_padding_mask,
#             #                memory_key_padding_mask=memory_key_padding_mask,
#             #                pos=pos, query_pos=query_pos,residual=residual)
            
#             # outputs[high_level_idx] = output_high
#             # if self.multi:
#             #     outputs[low_level_idx] = output_low
#             #     for i, w in enumerate(ws):
#             #         shape_low = (np.sqrt(ws_low[i].shape[0]), np.sqrt(ws_low[i].shape[0]))
#             #         ws_high[i] = ws_high[i].view(B, N, np.sqrt(T), np.sqrt(T))
#             #         ws_low[i] = ws_low[i].view(B, N, shape_low[0], shape_low[1])
#             #         ws_high[i] = F.interpolate(ws_high[i], shape_low).view(B, N, -1)
#             #         w[:, high_level_idx] = ws_high[i]
#             #         w[:, low_level_idx] = ws_low[i]
#             atten_layers.append(ws)
#             if self.return_intermediate:
#                 intermediate.append(self.norm(output))
#         if self.norm is not None:
...
class ThreeLevelTransformerDecoderLayer(nn.Module):

    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
                 activation="relu", normalize_before=False, self_attention=True, low_level_idx=None, global_level_idx=None):
        super().__init__()
        ...
        self.global_level_idx = global_level_idx
    ...
    ## Apply norm
    def forward_pre(self, tgt, high_memory, low_memory, global_memory,
                    tgt_mask: Optional[Tensor] = None,
                    memory_mask: Optional[Tensor] = None,
                    tgt_key_padding_mask: Optional[Tensor] = None,
                    memory_key_padding_mask: Optional[Tensor] = None,
        ...
        tgt2 = self.norm2(tgt)
        high_level_index = [i for i in range(tgt.shape[0]) if i not in self.low_level_index + self.global_level_idx]
        N_high, B, _ = high_memory.shape
        N_low, B, _ = low_memory.shape
        N_global, _, _ = global_memory.shape
        high_res = int(np.sqrt(N_high)), int(np.sqrt(N_high))
        low_res = int(np.sqrt(N_low)), int(np.sqrt(N_low))
        global_res = int(np.sqrt(N_global)), int(np.sqrt(N_global))
        ...
        global_tgt2 = tgt2[self.global_level_idx]
        global_tgt2, global_attn_weights = self.multihead_attn(query=self.with_pos_embed(global_tgt2, query_pos),
                                   key=self.with_pos_embed(global_memory, pos),
                                   value=global_memory, attn_mask=memory_mask,
                                   key_padding_mask=memory_key_padding_mask)
        global_attn_weights = global_attn_weights.view(B, -1, global_res[0], global_res[1])
        global_attn_weights = F.interpolate(global_attn_weights, low_res, mode='bilinear').view(B, -1, N_low)

        attns = torch.zeros(tgt.shape[1], tgt.shape[0], low_memory.shape[0]).to(tgt.device)
        attns[:, high_level_index] = high_attn_weights
        attns[:, self.low_level_index] = low_attn_weights
        attns[:, self.global_level_idx] = global_attn_weights
        tgt2 = torch.zeros_like(tgt2).to(tgt2.device)
        tgt2[self.low_level_index] = low_tgt2
        tgt2[high_level_index] = high_tgt2
        tgt2[self.global_level_idx] = global_tgt2
    ...
    def forward(self, tgt,  high_memory, low_memory, global_memory,
        ...
        if self.normalize_before:
            return self.forward_pre(tgt, high_memory, low_memory, global_memory, tgt_mask, memory_mask,
