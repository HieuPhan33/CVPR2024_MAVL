...
class MAVL(nn.Module):
    ...
    def __init__(self, config, ana_book, disease_book, concept_book):
        ...
        with torch.no_grad():
            ...
            print(self.n_disease, concept_book['input_ids'].shape[0])
    ...
    def forward(self, images, return_intermediate=True):
        '''
        ...
        sim = torch.einsum('bnd,ad->bna', l_emb, self.ana_book)
        if self.same_feature:
            out_features = out.contiguous().view(B, self.n_disease*self.n_concepts, -1) # B, N, H
            # query_embed_ N, H
            ...
            logits_scores = torch.stack([logits[:, k, k] for k in range(self.n_disease*self.n_concepts)], dim=-1)
            logits_normal = torch.stack([logits[:, k, k%self.n_concepts] for k in range(self.n_disease*self.n_concepts)], dim=-1)
            logits = F.softmax(torch.stack([logits_normal, logits_scores], dim=-1), dim=-1)
            logits = logits.contiguous().view(-1, self.n_disease, self.n_concepts, 2).contiguous().permute(0, 2, 1, 3)
        else:
            logits = torch.matmul(F.normalize(x_global, dim=-1), F.normalize(query_embed_, dim=-1).T) # B x N_disease*n_concepts
            logits = logits.view(B, self.n_disease, self.n_concepts).contiguous().permute(0, 2, 1)
        ...
        ## Computing
        weight = self.classifier.weight # N_concepts x d
        #concept_features = F.softmax(torch.stack([out*weight[0], out*weight[1]], dim=-1), dim=-1)[:, :, :, 1]
        concept_features = out*weight[1]
        concept_features = concept_features.view(B, self.n_disease, self.n_concepts, -1).sum(dim=-1)
        # concept_features = torch.matmul(out)
        if return_intermediate:
            return x, sim, concept_features, logits, out
        return x
