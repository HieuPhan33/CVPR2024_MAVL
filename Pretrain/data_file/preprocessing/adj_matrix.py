...
'''
Standard anatomical name: {
    'a': ['some altenative anatomical names'], 
    'm1': ['some location names'], 
    'm2': ['related locations'], 
    'sc':[some other alternatives]
}
'''
...
class LandmarkObservationAdjacentMatrix(Dataset):
    ...
    def __getitem__(self, idx):
        sid = self.sids[idx]
        df_sid = self.df_anatomy_label[self.df_anatomy_label['study_id'] == sid] # Iterate through all reports associated with the same study ID
...
# Check if an input anatomy x matches the reference anatomy set a based on the specified types
...
def create_adj_matrix(args):
    ...
    for index, row in tqdm(df_anatomy_label.iterrows(), total=df_anatomy_label.shape[0]):
        ...
        for k, v in DICT_ANATOMICAL_LANDMARKS.items():
            # Match anatomical term x with all possible reference anatomical names in v[..]
            flag = anatomy_to_landmark(x, v['a'], v['m1'], v['m2'], v['sc'], v['t'])
            if flag:
                landmark_name.append(k)
                break
        if (not flag) & (row.anatomy == 'unspecified'): # Disease exists but unspecified
            landmark_name.append('unspecified')
        elif (not flag) & (row.anatomy != 'unspecified'): # Note, for study id 0, obs catheter=23, it couldn't map right atrium to the landmark?
            landmark_name.append('other')
    ## A list of N varied anatomies are now standardized into N landmark names 
    ...
    sid_lst = []
    adj_mtx_lst = []
    ## Loop through each report, and get their observation-location presence in adj matrix format
